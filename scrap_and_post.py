...
def unsplashCrawl(keyword):
    ...
    driver.find_element_by_name("searchKeyword").send_keys(keyword[random.randint(0, len(keyword)-1)])
    pyautogui.press("enter")
    time.sleep(2)
    # imageload and get caption
    i = driver.find_element_by_css_selector("._2UpQX")
    i.click()
...
def getDownloadedImageName():
    ...
    time.sleep(2)
    # namelist = driver.find_element_by_xpath("//a[@id='file-link']").text
    namelist = driver.find_element_by_id("file-link").text
    ...
    pyperclip.copy(''.join(["/home/zeph/Pictures/", name]))
...
def hashtagGeneration(keyword):
    ...
    time.sleep(3)
    driver.find_element_by_xpath('//button[text()="Copy Mode"]').click()
    time.sleep(2)
...
options.add_experimental_option("prefs", {
    "download.default_directory": r"/home/zeph/Pictures",
...
ua = UserAgent()
options.add_argument(f'user-agent={ua.safari}')
driver = webdriver.Chrome(chrome_options=options, executable_path='/home/zeph/projects/instaposter/chromedriver')
